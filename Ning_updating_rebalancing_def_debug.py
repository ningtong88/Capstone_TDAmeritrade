...
# %%
def input_ticker(num=3, input_ticker=tuple(['GLD','SPY','TLT'])):
    num = 3
    ticker_list = {'tickers': input_ticker}
    co_no_ticker_com = crsp_db.raw_sql('SELECT permco, permno, ticker, comnam FROM crsp.stocknames WHERE ticker in %(tickers)s group by permco, permno, ticker, comnam order by 3 asc', params=ticker_list)
    return num,ticker_list,co_no_ticker_com
...
# %%
permno_df = crsp_db.raw_sql('select distinct permno from (SELECT permco, permno, ticker, comnam FROM crsp.stocknames WHERE ticker in %(tickers)s     and comnam like \'%%TRUST\' group by permco, permno, ticker, comnam order by 3 asc) a', params=ticker_list)
